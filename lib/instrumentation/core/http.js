...
module.exports = function initialize(agent, http) {
  ...
  shimmer.wrapMethod(http, 'http', 'request', function (request) {
    return agent.tracer.transactionProxy(function wrappedRequest(options, callback) {
      ...
      if (state) {
        ...
        if (!internalOnly) {
          var name        = 'External/' + options.hostname + requested.path
            , gatherer    = transaction.metrics.externalMetrics(options.hostname, 'http')
          ...
          if (requested.on) {
            requested.on('response', function (res) {
              res.once('end', function instrumentedFinish() {
                segment.end();
                transaction.measureWeb(requested.path, res.statusCode, segment.getDurationInMillis());
                transaction.end();
              });
            });
          }
